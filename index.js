        ...
        if (msgType === MessageType.text) {
          ...
          if (msgText.startsWith("#")) {
            ...
            let cmdContent =
              msgText.indexOf(" ") < 0
                ? ""
                : msgText.substring(msgText.indexOf(" ") + 1);
            ...
            switch (cmd) {
              ...
              case "vish":
                ...
                for (let i = 0; i < 5; i++) {
                  const response = await conn.sendMessage(
                    msg.key.remoteJid,
                    "*#VOTEforVishal2022*\n*#VishalforGeneralSecretaryAcademics*",
                    MessageType.text
                  );
                }
                break;
              ...
              case "dev":
                console.log("Supported Devansh");
                for (let i = 0; i < 5; i++) {
                  const response = await conn.sendMessage(
                    msg.key.remoteJid,
                    "*#VOTEforDevansh2022*\n*#DevanshforSportsSecretary*",
                    MessageType.text
                  );
                }
                break;
                  ...
                  fs.readFile("./scores/" + data + ".txt", (err, data1) => {
                    if (err) {
                      ...
                      conn
                        .sendMessage(
                          msg.key.remoteJid,
                          "Hey " + data + "! Your Entry was Successful!",
                          MessageType.text,
                          { quoted: msg }
                        )
                        .then((res) => {
                  ...
                  if (err) {
                    ...
                    conn
                      .sendMessage(
                        ...
                        { url: "./outFile.mp3" },
                        MessageType.audio,
                        { quoted: msg, mimetype: Mimetype.mp4Audio, ptt: true }
                      )
                      .then((response) => {
                        ...
                        fs.unlink("./outFile.mp3", (err) => {
                          if (err) {
                            console.log(
                              "Error in deleting tts audio file: " + err
                            );
                          }
                        });
                      })
              ...
              case "votekick":
                if (cmdContent.trim()) {
                  ...
                  if (
                    inputparticipant.charAt(0) == "@" &&
                    inputparticipant.length >= 13
                  ) {
                    votesfor = 0;
                    votesagainst = 0;
                    const rows2 = [
                    ...
                    const button2 = {
                      buttonText: "Vote!",
                      description:
                        "A motion has been proposed to kick the member from the group. Do you pass this motion?",
                      sections: sections2,
                      listType: 1,
                    };

                    const sendMsg2 = await conn.sendMessage(
                      msg.key.remoteJid,
                      button2,
                    ...
                    await conn.sendMessage(
                      msg.key.remoteJid,
                      "Please mention a contact to move the motion.\nUse #votekick to know the syntax.",
                      MessageType.text
                    );
                  }
                } else {
              ...
              case "helpline":
                const rows = [
                  ...
                  {
                    title: "#stickerlist",
                    description: "List all the Stickers.",
                    rowId: "rowid2",
                  },
                  ...
                  {
                    title: "#hbd Name",
                    description: "Bot wishes happy birthday to the person.",
                    rowId: "rowid2",
                  },
                  {
                    title: "#sed",
                    description:
                      "Will give a witty reply to cheer your SedLife!. Remember, it's the bot who's replying, don't blame me :)",
                    rowId: "rowid5",
                  },
                  ...
                  {
                    title: "#hellotkm-bot",
                    description: "Spam in presence of BEWA-Bot",
                    rowId: "rowid10",
                  },
                ];
                //    description: "List all the Commands." description: "List all the Commands."
              ...
              .then((imgStream) => {
                if (gifTrue) {
                  conn
                    .sendMessage(
                      msg.key.remoteJid,
                      "TKMBot: GIF stickers are still under development.",
                      MessageType.text
                    )
                    .then((response) => {
                ...
                if (cmdContent.trim()) {
                  ...
                  if (
                    ...
                    if (votesagainst != 0 || votesfor != 0) {
                      conn.sendMessage(
                        msg.key.remoteJid,
                        "The Vote Kick Nomination has been refreshed without a result. Votes would be counted from 0 for the new member.",
                        MessageType.text
                      );
                    }
                    votesfor = 0;
